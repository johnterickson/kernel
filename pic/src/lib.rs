...
pub fn get_mask() -> u8 {
    unsafe {
        inb(PIC1_DATA_IO_PORT)
    }
}

pub fn enable_irq(index: u8) {
    unsafe {
        let address = if index >= 8 { PIC2_DATA_IO_PORT } else {PIC1_DATA_IO_PORT};
        let mut mask = inb(address);
        mask = mask & (!(1 << index));
        outb(address, mask);
    }
}

pub fn disable_irq(index: u8) {
    unsafe {
        let address = if index >= 8 { PIC2_DATA_IO_PORT } else {PIC1_DATA_IO_PORT};
        let mut mask = inb(address);
        mask = mask | (1 << index);
